...
class trafic4cast_dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, source_root, target_root="precomuted_data",
        ...
        for city in cities:
            source_file_paths = source_file_paths + glob.glob(
                os.path.join(self.source_root, city, self.split_type,
                             '*.h5'))
        ...
        for city in cities:
            target_file_paths = target_file_paths + glob.glob(
                os.path.join(self.target_root, city, self.split_type,
                             '*.h5'))
    ...
    def precompute_clip(self, source_path, mode='writing'):
        ...
        if mode == 'writing':
            ...
            dset = f_target.create_dataset('array', (288, 9, 495, 436),
                                           chunks=(1, 9, 495, 436),
    ...
    def __len__(self):
        if self.split_type == 'test':
            ...
            return len(self.target_file_paths) * 263
