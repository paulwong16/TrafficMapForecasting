...
from config_res_unet_fusion import config
# from models.unet import UNet
from models.deep_resunet_fusion import Deep_Res_UNet_fusion
...
def train(model, train_loader, static_map, optim, device, writer, epoch, globaliter):
    model.train()
    running_loss = 0.0
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        inputs = torch.cat([inputs, static_map], dim=1)
        inputs = inputs / 255
...
def validate(model, val_loader, static_map, device, writer, globaliter, if_testtimes=False):
    total_val_loss = 0
    if if_testtimes:
        prefix = 'testtimes'
    else:
    ...
    with torch.no_grad():
        for i, data in enumerate(val_loader, 0):
            ...
            val_inputs = torch.cat([val_inputs, static_map], dim=1)
...
if __name__ == "__main__":
    ...
    n = len(dataset_train)
    split = config['subsample']
    if split is None:
        train_loader = torch.utils.data.DataLoader(dataset_train, shuffle=True, **config['dataloader'])
    else:
        indices = np.random.permutation(n)
        subset_sampler = torch.utils.data.sampler.SubsetRandomSampler(indices[:split])

        train_loader = torch.utils.data.DataLoader(dataset_train, sampler=subset_sampler, **config['dataloader'])
    val_loader = torch.utils.data.DataLoader(dataset_val, shuffle=True, **config['dataloader'])
    # val_loader_ttimes = torch.utils.data.DataLoader(dataset_val_ttimes, shuffle=True, **config['dataloader'])

    device = torch.device(config['device_num'] if torch.cuda.is_available() else 'cpu')

    # load static map
    f = h5py.File(config['static_map'], 'r')
    static_map = f.get('array')
    static_map = np.moveaxis(static_map, 2, 0)
    ch_, row_, col_ = static_map.shape
    static_map = static_map.reshape((1, ch_, row_, col_))
    static_map = torch.from_numpy(static_map).float()
    ...
    if config['cont_model_path'] is None:
        ...
        model = Deep_Res_UNet_fusion(**config['model']).to(device)
        # model = NestedUNet(36, 9).to(device)
    ...
    # load alrady trained model
        ...
        model = Deep_Res_UNet_fusion(**config['model']).to(device)
        # model = NestedUNet(36, 9).to(device)
        checkpoint = torch.load(os.path.join(cont_model_path, 'checkpoint.pt'),
                                         map_location=device)
        model.load_state_dict(checkpoint['net'])
        start_epoch = checkpoint['epoch']
        globaliter = checkpoint['globaliter']
    else:
    ...
    trainNet(model, train_loader, val_loader, device, static_map, start_epoch, globaliter)
